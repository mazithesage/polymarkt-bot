...
class TestKellyCriterion:
    ...
    def test_kelly_doesnt_blow_up_on_garbage_inputs(self):
        # regression: caught div-by-zero in prod when NO price was 0
        assert kelly_criterion(estimated_prob=0.0, market_price=0.50, bankroll=1000.0) is None
        assert kelly_criterion(estimated_prob=1.0, market_price=0.50, bankroll=1000.0) is None
        assert kelly_criterion(estimated_prob=0.50, market_price=0.0, bankroll=1000.0) is None
        assert kelly_criterion(estimated_prob=0.50, market_price=1.0, bankroll=1000.0) is None
        assert kelly_criterion(estimated_prob=-0.1, market_price=0.50, bankroll=1000.0) is None
        assert kelly_criterion(estimated_prob=1.5, market_price=0.50, bankroll=1000.0) is None
    ...
    def test_derived_price_near_zero(self):
        # When market_price ~1.0, derived NO price ~0 — should not crash
        result = kelly_criterion(
            estimated_prob=0.01, market_price=0.999999, bankroll=1000.0, min_edge=0.0001,
        )
        if result is not None:
            assert result.position_size > 0

    def test_confidence_matches_selected_prob(self):
        result = kelly_criterion(estimated_prob=0.70, market_price=0.50, bankroll=1000.0)
        assert result is not None
        assert result.confidence == 0.70

        result_no = kelly_criterion(estimated_prob=0.30, market_price=0.50, bankroll=1000.0)
        assert result_no is not None
        assert result_no.confidence == 0.70  # 1 - 0.30
...
class TestMultiOutcomeKelly:
    def test_finds_best_edges(self):
        results = multi_outcome_kelly(
            ...
            bankroll=1000.0, fraction=0.25, min_edge=0.02,
        )
...
class TestPositionLimits:
    def test_within_limits(self):
        assert check_position_limits(50.0, 30.0, 500.0) == 30.0

    def test_at_limit(self):
        assert check_position_limits(500.0, 50.0, 500.0) == 0.0

    def test_partial_fill(self):
        assert check_position_limits(480.0, 50.0, 500.0) == 20.0

    def test_zero_exposure(self):
        assert check_position_limits(0.0, 100.0, 500.0) == 100.0
