...
class TestSSVITotalVariance:
    def test_atm_variance(self):
        # At k=0: w(0) = theta/2 * (1 + sqrt(rho^2 + 1 - rho^2)) = theta
        k = np.array([0.0])
    ...
    def test_skew_with_negative_rho(self):
        # Negative rho => left wing (OTM puts) has higher variance
        k = np.array([-0.3, 0.3])
...
class TestFitSSVI:
    ...
    def test_fit_noisier_data(self):
        ...
        assert params.r_squared > 0.40  # more noise, lower bar
        assert params.theta > 0

    def test_fit_requires_minimum_strikes(self):
        with pytest.raises(ValueError):
            fit_ssvi(
                np.array([100.0, 110.0]), np.array([0.3, 0.35]),
                forward=100.0, time_to_expiry=1 / 12,
            )

    def test_fit_requires_positive_time(self):
        with pytest.raises(ValueError):
            fit_ssvi(
                np.array([90, 100, 110.0]), np.array([0.3, 0.25, 0.35]),
                forward=100.0, time_to_expiry=0.0,
            )
...
class TestExtractProbability:
    ...
    def test_deep_itm_high_prob(self):
        ...
        prob = extract_probability(
            params, spot=100.0, forward=100.0, time_to_expiry=1 / 12, strike=50.0,
        )
    ...
    def test_deep_otm_low_prob(self):
        params = SSVIParams(theta=0.04, rho=0.0, phi=0.5, r_squared=0.9)
        prob = extract_probability(
            params, spot=100.0, forward=100.0, time_to_expiry=1 / 12, strike=200.0,
        )
        assert prob.probability_above < 0.20

    def test_zero_tte_returns_half(self):
        params = SSVIParams(theta=0.04, rho=0.0, phi=0.5, r_squared=0.9)
